...
I was tasked to prepare some data for a recommendation system to help people find movie reviews and related movies. I extracted data from two different sources: The New York Times API and The Movie Database and merged the data together.
...
- Update your .env file with the New York Times and The Movie Database API keys
...
**Part 1: Access the New York Times API**
- I consulted the New York Times Article Search API documentation to build my query URL using the variables included in the starter code.
...
- The Article Search API limits results to 10 per page and I retrieved 200. To do this, I create a for loop to loop through 20 pages (starting from page 0). Inside the loop, I performed the following actions:
...
- Converted reviews_list to a Pandas DataFrame using json_normalize()
- Extracted the movie title from the "headline.main" column and saved it to a new column "title" using the Pandas apply() method and the following lambda function:
...
- Used the supplied extract_keywords function to convert the "keywords" column from a list of dictionaries to strings using the apply() method.
- Created a list called titles from the "title" column using to_list(). These titles will be used in the query for The Movie Database.
...
- I used the titles list created in Part 1 to perform your queries with The Movie Database.

- I created an empty list called tmdb_movies_list to store the results from your API requests. This will contain a list of dictionaries.
- I created a variable called request_counter and initialized it with the value of 1. This counter does the following:
    - Increment by one every time you iterate through the titles list.
    - Use time.sleep(1) when it reaches a multiple of 50.
    - Print a message to indicate that the application is sleeping.

- Looped through the titles list created from the movie reviews DataFrame, and performed the following actions:
    ...
    - Use a try clause that performs the following actions:
        - Collected the movie ID from the first result.
        - Made a GET request using the movie query (starting with https://api.themoviedb.org/3/movie/) and movie ID to retrieve the full movie details in JSON format.
        - Extracted the genre names from the results into a list called genres.
        - Extracted the spoken_languages' English name from the results into a list called spoken_languages.
        - Extracted the production_countries' name from the results into a list called production_countries.
        - Created a dictionary with the following results: title, original_title, budget, original_language, homepage, overview, popularity, runtime, revenue, release_date, vote_average, vote_count, as well as the genres, spoken_languages, and production_countries lists I created.
        - Appended this dictionary to tmdb_movies_list.
        - Printed out the name of the movie and a message to indicate that the title was found.
    - Used the except clause to print out a statement if a movie is not found.

- Previewed the first five results in JSON format using json.dumps with the argument indent=4 to format the data.
- Converted the results to a DataFrame called tmdb_df with pd.DataFrame().
...
- The genres, spoken_languages, and production_countries columns were saved as lists, but the columns need to be converted be strings without the list characters ([, ], and '). To fix these columns, I performed the following actions:
    - Created a list of the columns that need fixing called columns_to_fix.
    - Created a list of characters to remove called characters_to_remove.
    - Looped through columns_to_fix and do the following:
        - Used astype() to convert the column to a string.
        - Looped through the characters_to_remove and use the Pandas str.replace() method to remove the character from the string.
    - Printed the head of the updated DataFrame to confirm the list characters were removed.
- Deleted any duplicate rows and reset the index.
- Exported data to a CSV file without the DataFrame's index.
